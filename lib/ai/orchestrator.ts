...
export class AIOrchestrator {
    ...
    private async checkBudgetAndLimits(userId: string, tier: AITier): Promise<AITier> {
        let currentTier = tier;

        try {
            // 1. Global Circuit Breaker Check
            // Calculate global spend today
            const { data: globalUsage, error: globalError } = await this.supabase
                .from('ai_usage_logs')
            ...
            if (!dailyError && dailyCount !== null && dailyCount >= strategy.dailyLimit) {
                ...
                return this.checkBudgetAndLimits(userId, currentTier === 'PRO' ? 'FLASH' : 'LITE');
            }
            ...
            if (strategy.weeklyLimit) {
                ...
                const { count: weeklyCount, error: weeklyError } = await this.supabase
                    .from('ai_usage_logs')
                    .select('*', { count: 'exact', head: true })
                    .eq('user_id', userId)
                    .eq('tier', currentTier)
                ...
                if (!weeklyError && weeklyCount !== null && weeklyCount >= strategy.weeklyLimit) {
                    ...
                    return this.checkBudgetAndLimits(userId, 'FLASH');
                }
            }
        } catch (e: any) {
            console.error(`[AI ORCHESTRATOR] Orchestration check failed, proceeding with original tier:`, e.message);
            // Fallback: proceed with requested tier if check fails, to avoid blocking service
        }

        return currentTier;
    }

    private async logUsage(userId: string, tier: AITier, model: string, intent: string, tokens: number) {
        try {
    ...
    async generate(params: {
        ...
        const finalTier = await this.checkBudgetAndLimits(params.userId, params.tier);
        ...
        // Ensure no raw image tags or base64 bleed into the output
        const cleanedOutput = text.trim();

        // 4. Usage Logging
        // Estimating tokens (approximate word count * 1.3)
        const estTokens = (cleanPrompt.length + cleanedOutput.length) / 4 * 1.3;
        await this.logUsage(params.userId, finalTier, strategy.model, params.intent, estTokens);
