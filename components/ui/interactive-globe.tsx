...
interface GlobeProps {
    ...
    markers?: { lat: number; lng: number; label?: string; id?: any; emoji?: string }[];
...
export function InteractiveGlobe({
    ...
    const markerPositionsRef = useRef<Map<any, { x: number; y: number }>>(new Map());

    // Generate globe dots (land approximation via density sampling)
    const dotsRef = useRef<[number, number, number][]>([]);
    const targetRotRef = useRef<{ rx: number, ry: number } | null>(null);

    useEffect(() => {
        if (activeMarkerId !== undefined && activeMarkerId !== null) {
    ...
    const draw = useCallback(() => {
        ...
        if (!dragRef.current.active) {
            if (targetRotRef.current) {
                rotXRef.current += (targetRotRef.current.rx - rotXRef.current) * 0.05;

                let diffY = targetRotRef.current.ry - rotYRef.current;
                while (diffY > Math.PI) diffY -= 2 * Math.PI;
                while (diffY < -Math.PI) diffY += 2 * Math.PI;
                rotYRef.current += diffY * 0.05;
            } else {
        ...
        for (const marker of markers) {
            ...
            // Label
            if (marker.label) {
                ctx.font = marker.id === activeMarkerId ? "bold 13px system-ui, sans-serif" : "11px system-ui, sans-serif";
                ctx.fillStyle = marker.id === activeMarkerId ? "rgba(255, 255, 255, 1)" : markerColor.replace("1)", "0.6)");
                ctx.fillText(marker.label, sx + 10, sy + 4);
            }
            if (marker.id !== undefined) {
                newMarkerPositions.set(marker.id, { x: sx, y: sy });
            }
        }
        markerPositionsRef.current = newMarkerPositions;

        animRef.current = requestAnimationFrame(draw);
    }, [dotColor, arcColor, markerColor, autoRotateSpeed, connections, markers, activeMarkerId]);
    ...
    const handleClick = useCallback((e: React.MouseEvent, isDoubleClick: boolean) => {
        const canvas = canvasRef.current;
        if (!canvas) return;
        ...
        let clickedMarkerId: any = null;
        let minDistance = 20; // 20px hit radius

        markerPositionsRef.current.forEach((pos, id) => {
        ...
        if (clickedMarkerId !== null) {
            ...
            if (marker) {
                if (isDoubleClick && onMarkerDoubleClick) {
                    onMarkerDoubleClick(marker);
                } else if (!isDoubleClick && onMarkerClick) {
                    onMarkerClick(marker);
                }
            }
        }
    }, [markers, onMarkerClick, onMarkerDoubleClick]);
    ...
    return (
        <canvas
            ...
            onClick={(e) => {
                // Ignore clicks if we just dragged
                const dx = Math.abs(e.clientX - dragRef.current.startX);
                const dy = Math.abs(e.clientY - dragRef.current.startY);
                if (dx > 5 || dy > 5) return;

                // We use detail to distinguish single click here if needed, but we also pass onDoubleClick handler below
                // Actually, native onClick fires on double click too (twice).
                // Let's rely on standard onDoubleClick for double clicks.
                handleClick(e, false);
            }}
            onDoubleClick={(e) => {
                handleClick(e, true);
