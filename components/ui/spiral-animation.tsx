...
// Animation Constants - Shared across classes to avoid property access issues
const STAR_CONFIG = {
    numberOfStars: 5000,
    cameraZ: -400,
...
class AnimationController {
    ...
    private createStars() {
        ...
        for (let i = 0; i < STAR_CONFIG.numberOfStars; i++) {
            this.stars.push(new Star(random))
        }
    }
    ...
    public spiralPath(p: number): Vector2D {
        ...
        return new Vector2D(
            r * Math.cos(theta),
            r * Math.sin(theta) + STAR_CONFIG.startDotYOffset
        )
    }
    ...
    public showProjectedDot(position: Vector3D, sizeFactor: number) {
        const t2 = this.constrain(this.map(this.time, STAR_CONFIG.changeEventTime, 1, 0, 1), 0, 1)
        const newCameraZ = STAR_CONFIG.cameraZ + this.ease(Math.pow(t2, 1.2), 1.8) * STAR_CONFIG.cameraTravelDistance

        if (position.z > newCameraZ) {
            const dotDepthFromCamera = position.z - newCameraZ

            // 3D -> 2D投影公式
            const x = STAR_CONFIG.viewZoom * position.x / dotDepthFromCamera
            const y = STAR_CONFIG.viewZoom * position.y / dotDepthFromCamera
    ...
    private drawStartDot() {
        if (this.time > STAR_CONFIG.changeEventTime) {
            const dy = STAR_CONFIG.cameraZ * STAR_CONFIG.startDotYOffset / STAR_CONFIG.viewZoom
            const position = new Vector3D(0, dy, STAR_CONFIG.cameraTravelDistance)
    ...
    public render() {
        ...
        // 计算时间参数
        const t1 = this.constrain(this.map(this.time, 0, STAR_CONFIG.changeEventTime + 0.25, 0, 1), 0, 1)
        const t2 = this.constrain(this.map(this.time, STAR_CONFIG.changeEventTime, 1, 0, 1), 0, 1)
    ...
    private drawTrail(t1: number) {
        for (let i = 0; i < STAR_CONFIG.trailLength; i++) {
            const f = this.map(i, 0, STAR_CONFIG.trailLength, 1.1, 0.1)
...
class Star {
    ...
    constructor(random: () => number) {
        ...
        this.expansionRate = 1.2 + random() * 0.8 // 增加扩散率从0.8-1.2到1.2-2.0
        this.finalScale = 0.7 + random() * 0.6 // 0.7-1.3之间的最终尺寸

        this.dx = this.distance * Math.cos(this.angle)
        this.dy = this.distance * Math.sin(this.angle)

        this.spiralLocation = (1 - Math.pow(1 - random(), 3.0)) / 1.3
        const zMin = 0.5 * STAR_CONFIG.cameraZ;
        const zMax = STAR_CONFIG.cameraTravelDistance + STAR_CONFIG.cameraZ;
        this.z = zMin + random() * (zMax - zMin);

        const lerp = (start: number, end: number, t: number) => start * (1 - t) + end * t
        this.z = lerp(this.z, STAR_CONFIG.cameraTravelDistance / 2, 0.3 * this.spiralLocation)
    ...
    render(p: number, controller: AnimationController) {
        ...
        if (q > 0) {
            ...
            // 将2D屏幕坐标转换为3D空间坐标
            const vx = (this.z - STAR_CONFIG.cameraZ) * screenX / STAR_CONFIG.viewZoom;
            const vy = (this.z - STAR_CONFIG.cameraZ) * screenY / STAR_CONFIG.viewZoom;
