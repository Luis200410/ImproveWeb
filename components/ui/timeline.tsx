...
export const Timeline = ({ data, title = "The Journey", description = "A chronological overview of your habit landscape and progression.", isDailyProportional = false }: { data: TimelineEntry[], title?: string, description?: string, isDailyProportional?: boolean }) => {
    ...
    return (
        ...
        >
            ...
            <div ref={ref} className={`relative max-w-7xl mx-auto ${isDailyProportional ? 'h-[2400px] mt-10' : 'pb-20'}`}>
                {isDailyProportional && (
                ...
                {data.map((item, index) => {
                    ...
                    return (
                        <div
                            ...
                            style={isDailyProportional ? { top: topPosition } : {}}
                        >
                            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
                                ...
                                <h3 className="hidden md:block text-xl md:pl-20 md:text-4xl font-bold text-white/40 font-serif">
                                    {item.title}
                                </h3>
                            </div>

                            <div className={`relative pl-20 ${isDailyProportional ? 'md:pl-20 pt-10 mt-2' : 'pr-4 md:pl-4'} w-full`}>
                                <h3 className="md:hidden block text-2xl mb-4 text-left font-bold text-white/40 font-serif">
                                    {item.title}
                                </h3>
                                {item.content}
                            </div>
