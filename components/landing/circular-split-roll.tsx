...
function CircularSplitRollComp({
  ...
  imageCardWidth = 240,
  imageCardHeight = 260,
  titleSize = "clamp(20px, 2.5vw, 48px)",
      ...
      const ctx = gsap.context(() => {
        ...
        const render = (scrollProgress: number) => {
          ...
          if (width < DESKTOP_WIDTH) {
            factor = Math.max(width / DESKTOP_WIDTH, 0.55);
          }

          const leftRadiusScaledX = leftRadiusX * factor * 0.9;
          const leftRadiusScaledY = leftRadiusY * factor * 0.9;
          const rightRadiusScaledX = rightRadiusX * factor * 0.9;
          const rightRadiusScaledY = rightRadiusY * factor * 0.9;
  ...
  return (
    ...
    >
      ...
      <div
        ref={stickyRef}
        aria-hidden="false"
        className={`relative h-screen w-full overflow-hidden ${reducedMotion ? "hidden" : "max-sm:hidden"}`}
      >
            ...
            <div className="relative h-[78vh]">
              {safeItems.map((item) => (
                <Link
                  key={item.id}
                  href={`/apps/${item.slug}`}
                  className="circular-scroll-showcase__left-item pointer-events-auto absolute left-1/2 top-1/2 w-full origin-center whitespace-nowrap text-center text-(length:--css-title-size,clamp(20px,2.5vw,48px)) font-black uppercase leading-none tracking-tight opacity-0 will-change-[transform,opacity] transition-colors cursor-pointer group"
                >
                  <span
      ...
      <div className={`w-full px-5 py-12 max-sm:px-4 max-sm:py-8 ${reducedMotion ? "block" : "hidden max-sm:block"}`}>
        <div className="max-w-xl mx-auto text-center space-y-3 mb-8">
          <span className="kicker text-[#FF02E8] text-xs font-mono font-bold tracking-widest uppercase">Ecosystem Selector</span>
          <h2 className="title-huge text-2xl sm:text-4xl font-black uppercase text-white tracking-tight">The 7 Integrated Applications</h2>
        </div>
        <div className="mx-auto grid w-full max-w-md grid-cols-1 sm:grid-cols-2 gap-4">
          {safeItems.map((item) => (
            ...
            >
              <div
                className={`relative aspect-[16/9] w-full overflow-hidden rounded-[20px] bg-[#0c0a14] border border-white/15 p-4 flex items-center gap-4 shadow-xl transition-all duration-300 ${gridImageClassName}`}
                onMouseEnter={(e) => {
                  e.currentTarget.style.borderColor = item.accentHex;
              ...
              >
                <div className="h-12 w-12 shrink-0 flex items-center justify-center p-1.5 rounded-xl bg-black/40 border border-white/10">
                  <img
                    src={item.logoUrl}
                    alt={item.alt}
                ...
                <div className="flex flex-col justify-center min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-mono font-bold" style={{ color: item.accentHex }}>
                      {item.number}
                    </span>
                    <span className="text-sm font-black uppercase tracking-tight text-white group-hover:text-white/90 truncate">
                      {item.title}
                    </span>
                  </div>
                  {item.tagline && (
