...
import { ImproveLogo, AnimatedLogoSvg } from "@/components/landing/improve-logo";

/* ------------------------------------------------------------------ *
 * Inlined focus trap helper
 * ------------------------------------------------------------------ */
    ...
    <div ref={rootRef}>
      <header className={`fixed top-0 left-0 right-0 z-70 flex h-20 items-center justify-between px-4 sm:px-12 backdrop-blur-md bg-black/40 border-b border-white/10 ${headerClassName}`}>
        ...
        <button
          ...
          className="flex size-10 cursor-pointer flex-col items-center justify-center gap-1.5 max-sm:size-9"
        >
          <span
...
const SOCIAL_ICONS: Record<string, ReactNode> = {
  instagram: (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 sm:h-7 sm:w-7">
...
function CustomNavbar({
  ...
  return (
    <div style={{ backgroundColor: overlayBg }} className="flex min-h-screen w-full flex-col justify-between gap-6 px-4 sm:px-12 md:px-20 py-8 pt-24 text-white">
      <div className="flex items-center justify-between gap-8 md:gap-14 max-lg:flex-col max-lg:items-center max-lg:justify-center max-lg:gap-8 my-auto w-full max-w-7xl mx-auto">
        <div className="flex flex-col justify-center gap-2 sm:gap-4 md:gap-5 w-1/2 max-lg:w-full max-lg:items-center max-lg:text-center">
          {links.map((link, index) => (
            <div key={link.label} ref={setLinkRef(index)} className="z-60 text-[6.5vw] max-lg:text-[8vw] max-sm:text-[9.5vw] font-black uppercase tracking-tight leading-[1.05]" style={{ opacity: 0, transform: `translateY(${linkOffsetY}px)` }}>
        ...
        <div
          ...
          className="w-1/2 max-lg:w-full flex items-center justify-center p-2"
        >
          <AnimatedLogoSvg
            triggerKey={isOpen}
            duration={1.2}
            ...
            className="h-[36vw] min-h-[340px] max-h-[560px] w-auto max-lg:h-[38vh] max-lg:min-h-[260px] max-lg:max-h-[360px] max-sm:h-[30vh] max-sm:min-h-[200px] hover:scale-105 transition-transform duration-500"
          />
        </div>
      </div>
