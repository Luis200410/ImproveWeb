    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full">
            ...
            <div className="p-6 rounded-3xl bg-[#0A0A0A] border border-white/10 flex flex-col justify-between">
                <div>
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <span className={cn("text-lg font-mono", prodScore >= 0 ? "text-emerald-400" : "text-red-400")}>
                            {prodScore.toFixed(1)}
                        </span>
                    </div>
            ...
            <div className="p-6 rounded-3xl bg-[#0A0A0A] border border-white/10 relative overflow-hidden flex flex-col justify-between">
                ...
                <h3 className="text-xs uppercase tracking-widest text-white/40 mb-2">Task Stats</h3>

                <div className="flex items-center justify-between relative py-2 gap-4">
                    {/* Circular Progress for Completion Rate */}
                    <div className="relative shrink-0 w-20 h-20">
                        <svg className="w-20 h-20 rotate-[-90deg]">
                            <circle className="text-white/5" strokeWidth="6" stroke="currentColor" fill="transparent" r="36" cx="40" cy="40" />
                            <circle className="text-amber-400 transition-all duration-1000 ease-out" strokeWidth="6" strokeDasharray="226"
                                strokeDashoffset={226 - (226 * taskCompletion / 100)}
                    ...
                    <div className="grid grid-cols-2 gap-2 flex-1 pt-2 border-t border-white/5">
                        <div>
                            <div className="text-[9px] uppercase tracking-wider text-white/30 mb-1">Avg Duration</div>
                            <div className="text-xs font-medium text-blue-400">{taskMetrics?.avgDurationHours.toFixed(1)} hrs</div>
                        </div>
                        <div className="text-right">
                            <div className="text-[9px] uppercase tracking-wider text-white/30 mb-1">Total</div>
                            <div className="text-xs font-medium text-emerald-400">{taskMetrics?.totalTasks}</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            {/* Project Stats */}
            <div className="p-6 rounded-3xl bg-[#0A0A0A] border border-white/10 flex flex-col justify-between">
                <h3 className="text-xs uppercase tracking-widest text-white/40 mb-4">Project Metrics</h3>
